    ...
    <main className="py-10 sm:py-14">
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <section className="mt-14 grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-xl bg-primary/5 border border-primary/15 p-6"><h2 className="text-xl font-bold mb-3">{isCs ? 'Chybí požadovaný jazyk?' : 'Missing the language you need?'}</h2><p className="text-sm text-foreground/75 mb-5">{isCs ? 'Najděte titulky odpovídající release a přeložte je se zachováním časování. První kompletní soubor je zdarma.' : 'Find a subtitle matching the release, then translate it while preserving timing. Your first complete file is free.'}</p><Link href={`${prefix}/translate?from=subtitle-catalog`} className="inline-flex items-center gap-2 text-primary font-semibold">{isCs ? 'Přeložit první soubor zdarma' : 'Translate first file free'} <ArrowRight className="h-4 w-4" /></Link></div>
        </section>
