...
export async function LanguageCatalogPage({ slug, locale = 'en' }: { slug: string; locale?: 'en' | 'cs' }) {
  ...
  return <main className="py-12 sm:py-16"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, '\\u003c') }} /><div className="container mx-auto px-4 max-w-6xl">
    ...
    <section className="mt-14 rounded-2xl border p-7"><h2 className="text-2xl font-bold mb-3">{isCs ? 'Jazyk chybí u vašeho release?' : 'Is this language missing for your release?'}</h2><p className="text-muted-foreground mb-5">{isCs ? 'Najděte správně synchronizovaný soubor a přeložte ho při zachování časových značek. První kompletní soubor je zdarma.' : 'Find a correctly synchronized file and translate it while preserving timestamps. Your first complete file is free.'}</p><Link href={`${prefix}/translate?from=language-catalog`} className="font-semibold text-primary hover:underline">{isCs ? 'Přeložit první soubor zdarma →' : 'Translate first file free →'}</Link></section>
  </div></main>
}
