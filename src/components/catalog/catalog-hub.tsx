...
export function CatalogHub({ locale = 'en', type, view }: { locale?: 'en' | 'cs'; type?: CatalogMediaType; view?: 'popular' | 'latest' }) {
  ...
  return (
    <main className="py-12 sm:py-16"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, '\\u003c') }} /><div className="container mx-auto px-4 max-w-6xl">
      <header className="max-w-3xl mb-10"><div className="inline-flex items-center gap-2 text-primary font-semibold mb-4">{type === 'tv' ? <Tv className="h-5 w-5" /> : <Film className="h-5 w-5" />}{isCs ? 'Ověřovaný katalog titulků' : 'Verified subtitle catalog'}</div><h1 className="text-4xl sm:text-5xl font-bold tracking-tight mb-5">{heading}</h1><p className="text-lg text-muted-foreground leading-relaxed">{isCs ? 'Otevřete konkrétní titul a porovnejte aktuální vzorek jazyků a release variant z OpenSubtitles. Katalog obsahuje ověřené titulové stránky a rozšiřuje se podle skutečné poptávky.' : 'Open a title to compare a current sample of languages and release variants from OpenSubtitles. The catalog contains verified title pages and expands according to real demand.'}</p></header>
